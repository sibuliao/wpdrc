...
<body>
	...
	<div class="masthead" style="margin-top: 5px">
					...
					<div class="modal-body">
						<div class="input-group">
							<input type="text" class="form-control" placeholder="商品名称"
								aria-describedby="basic-addon1" id="input_name" required>
						</div>

						<div class="input-group">
							<input type="text" class="form-control" placeholder="价格"
								aria-describedby="basic-addon1" id="input_price" required>
						</div>
						...
						<div class="input-group">
							...
							<h4>
								<span class="label label-default" id="span_category"></span>
							</h4>
		...
		<!-- Button trigger modal -->
		<button type="button" class="btn btn-primary btn-lg"
			data-toggle="modal" data-target="#myModal3" id="btn_modify"
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="input-group">
							<input type="text" class="form-control" placeholder="商品名称"
								aria-describedby="basic-addon1" id="modify_input_name" required>
						</div>

						<div class="input-group">
							<input type="text" class="form-control" placeholder="价格"
								aria-describedby="basic-addon1" id="modify_input_price" required>
						</div>
						...
						<div class="input-group">
							<div class="dropdown">
								...
								<ul class="dropdown-menu" aria-labelledby="dropdownMenu3"
									id="modify_ul_category">
								</ul>
							</div>
							<h4>
								<span class="label label-default" id="modify_span_category"></span>
							</h4>
							<input type="hidden" id="modify_hidden_category_id" /> <input
								type="hidden" id="modify_hidden_product_id" />
						</div>
					</div>
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary"
							onclick="updateProduct(this)">保存</button>
					</div>
				</div>
			</div>
		</div>
 		...
 		function loadCategoryForModify(categoryId){
 			$.ajax({url: '<%=request.getContextPath()%>/category/list.do',
				...
				success : function(data) {
					if (data.status == 0) {
						$.each(data.data, function(index, value) {
							$("#modify_ul_category").append(
									'<li><a href="#" onclick="choseCategoryForModify('
											+ value.id + ', \'' + value.name
											+ '\')">' + value.name
											+ '</a></li>');
